...
import { SelectedSong, HostProps } from "../../Instances/Instances";
import {
  saveNewPassword,
  searchNewSong,
  changeRoomName,
...
const Host = ({ userInfo, _id, set_id }: HostProps) => {
  ...
  //All those useEffect could be refactored for sure.
  useEffect(scrollToBottom, [addedSong]);
  useEffect(() => {
    //onload / refresh
    const sessionRoom = JSON.parse(
  ...
  useEffect(() => {
    if (selectedSong.name !== undefined) {
      ...
      sessionStorage.setItem(
        "songList",
        JSON.stringify({ _id: _id, songs: addedSong })
      );
    }
  }, [selectedSong]);
  useEffect(() => {
    ...
    socket.on("receive_data", async (data) => {
      ...
      //stop the listeners otherwise it will multiply the responses.
  ...
  const handleRoomChange = async (e: any) => {
    if (e.key === "Enter") {
      ...
      if (checkingChange) {
        ...
        sessionStorage.setItem(
          "selectedRoom",
          JSON.stringify({ room: newRoom, readOnly: true })
        );
      } else {
  ...
  const handleCopyButton = async () => {
    ...
    navigator.clipboard.writeText(encodeURI(`${rootUrl}room/${newRoom}`));
  };
  //function for when the host decides to change the playlist.
...
//i feel sorry for whoever reads this. This looks like a Hadouken:
// https://external-preview.redd.it/6TTm4tS5pMCiq2KlgviubpGKt1QzvetOi2Z6Z3lB1Nc.jpg?auto=webp&s=c1df961d84c8d76048a93c1d261e425d3737d779
  return (
    <>
      <div className="hostMenu">
          ...
          <div className="shareMenu">
            ...
            {readOnly ? (
              <div className="shareBar">
                <input
                  id="urlInput"
                  onKeyPress={handleRoomChange}
                  readOnly={true}
                  value={`${rootUrl}room/${newRoom}`}
                ></input>
                <img
                  src={svgInfo}
                  alt=""
              ...
              <div className="shareBar">
                <input
                  ...
                  onKeyPress={handleRoomChange}
                  readOnly={readOnly}
                  placeholder={`${rootUrl}room/${newRoom}`}
                ></input>
                <img
                  src={svgInfo}
                  alt=""
            ...
            {hovered && (
              <span id="information">
                Select the room name you want. <br />
                If you select none, your Spotify user ID will be used. <br />
                Room name only allows alphanumeric characters.
                <br />
                You won't be able to change the room after setting it.
              </span>
            )}
            <button onClick={handleCopyButton}>copy</button>
          </div>
          {passGiven === false && (
            <form className="setPasswordForm" onSubmit={setPassword}>
              <input
                name="password"
                type="password"
                placeholder="Password for your playlist"
              ></input>
        ...
        {passGiven && (
          <div className="addedSongsList typeHost">
            <ul>
      ...
      <div className="backMenu">
        <button className="logoutButton" onClick={handleRemoveHost}>
          logout
        </button>
        <button className="logoutButton" onClick={changePlaylist}>
          Change playlist
        </button>
      </div>
    </>
