...
import { User, Playlist, GeneralPlaylist } from "../../Instances/Instances";
import {
  getNewToken,
...
const Menu = () => {
  ...
  useEffect(() => {
    ...
    const sessionPlaylist = JSON.parse(
      sessionStorage.getItem("playlistSelected") || "{}"
    );
    if (!sessionUser.hostId) {
      ...
      sessionPlaylist.playlistSelected &&
        setPlayListSelected(sessionPlaylist.playlistSelected);
    }
  }, []);
  ...
  const getPlaylists = async () => {
    try {
      ...
      const arr = playlistData.items.filter((el: GeneralPlaylist) => {
        return el.owner.id === userInfo.id;
      });
      setPlayLists((prev) => {
        return [...prev, ...arr];
      });
    } catch (error) {
      alert("Something happened while loading the playlists.");
    }
  };
  ...
  const handlePlaylistSubmit = async (e: React.MouseEvent<HTMLFormElement>) => {
    try {
      ...
      if (playlistName !== "New playlist") {
        const playlist = playLists.filter((el) => {
      ...
      sessionStorage.setItem(
        "playlistSelected",
        JSON.stringify({ playlistSelected: true })
      );
    } catch (error) {
      alert("Error while choosing the playlist.");
    }
  };
  ...
  return (
    <div className="mainContainer">
      ...
      {playLists.length === 0 && playListSelected === false && (
        <button onClick={getPlaylists}>Load or create a playlist</button>
      )}
      {playLists.length > 0 && playListSelected === false && (
            ...
            <div className="formInputAndButton">
              <select name="playlist">
                ...
                {playLists.map((playlist, index) => {
                  return <option key={index}>{playlist.name}</option>;
                })}
              </select>
              <button type="submit" className="playlistSelectorButton">
                START sharing
              </button>
            </div>
          </form>
      ...
      {playListSelected && (
        <Host set_id={set_id} _id={_id} userInfo={userInfo}></Host>
      )}
    </div>
  );
};
